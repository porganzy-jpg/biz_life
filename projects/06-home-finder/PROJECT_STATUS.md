# HomeFinder v2.0 - 프로젝트 현황

**최종 업데이트**: 2026-02-18

## 개요
마지막 집 찾기 - 부동산 매물 검색 + 분석 플랫폼. 건물(아파트/빌라)과 **토지** 매물을 4차원 채점 시스템으로 평가하고, 카카오맵 시각화 + 필터 검색 + 후보 관리를 제공. v2.0에서 토지 등록/수정 CRUD UI와 검색필터 강화(건물/토지 분류별 동적 전환)를 추가.
...
## 토지 CRUD UI (v2.0 신규, 2026-02-18)

### 매물 등록 폼 (land_form.html)
- 토지/건물 겸용 등록/수정 폼
- 토지 전용 필드: 지목, 용도지역, 건폐율, 용적률, 접도, 지형
- 수정 모드: 기존 데이터 자동 로드 + 수정 저장
- 주소 입력 시 카카오 API 좌표 자동 변환

### 매물 상세 (property_detail.html)
- 토지/건물별 다른 정보 테이블 동적 전환
- 수정/삭제 버튼 + 후보 추가 + 재채점
- 레이더 차트 라벨 토지/건물 자동 분기

### 검색필터 강화
- 건물/토지 분류 선택 시 필터 패널 동적 전환
- 건물: 연식, 층수, 향, 세대수 필터
- 토지: 지목, 용도지역, 건폐율, 용적률 필터
...
| **v2.0** | **2026-02-18** | **토지 등록/수정 CRUD UI + 검색필터 강화 (건물/토지 동적 전환)** |
