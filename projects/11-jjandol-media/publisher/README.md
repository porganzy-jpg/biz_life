...
**닫는 두 컷(쉼 + 시그니처)은 순환에서 빼고 항상 마지막 사진을 쓴다.** 순환에 맡기면
인덱스가 앞으로 되감겨, 여운을 남겨야 할 자리에 방금 나온 사진(라면 냄비)이 재등장한다.
마지막 사진은 창밖이나 빈 방처럼 정적인 컷으로 찍어 두는 게 좋다.

`photos/` 는 .gitignore 에 있다. 저장소에는 사진이 없으므로 clone 직후 `--video` 를 돌리면
카드로 렌더된다는 경고가 뜬다. 정상이다.
