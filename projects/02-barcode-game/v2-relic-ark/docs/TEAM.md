...
## 0.5 교본 (전문성) — `docs/curriculum/`
모든 에이전트는 `00_COMMON.md`, `07_AI_GAMEDEV.md`(AI 게임 제작 실전: 사례·워크플로우·공개 정책·마케팅), 자기 직군 교본을 성경보다 먼저 읽는다. 교본은 판단 기준(인기 게임의 원리, 우리가 원하는 게임, 직군별 원칙·사례·절차·자가 검수)이고, 성경은 내용(세계관·규칙)이다. 보고서에 적용 원칙을 번호로 인용한다.
...
- [ ] 완료 기준을 하나씩 대조.
- [ ] 보고서에 교본 원칙 인용(번호)이 있고, 직군 교본 §5 자가 검수 5문항에 답했음. 이탈 시 이유 기재.
- [ ] PM 재현: 보고서를 믿지 않고 직접 실행/열어서 확인한 항목을 기록.
- [ ] 마지막 질문: 이 산출물이 게임을 더 재미있게 했는가.
- [ ] AI 공개 관리표(07 §6) 갱신 필요 여부: 플레이어가 보고 듣는 AI 생성물이 새로 생겼는가.
- [ ] 이번 스프린트에 보여줄 스크린샷/클립 1개가 나왔는가(07 W6).
시나리오
