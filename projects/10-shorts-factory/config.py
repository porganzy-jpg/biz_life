...
# 정지 이미지를 테이크로 쓸 때의 프레이밍.
# 영상 테이크는 @초가 "원본의 다른 순간"을 가리키지만, 정지 이미지엔 다른 순간이 없다.
# 그래서 이미지에서는 @초를 "사진의 어느 부분을 보여줄지"로 해석한다.
# 같은 사진 한 장에서 전체샷/얼굴클로즈업/좌우 치우친 컷을 뽑아내 반복 체감을 줄인다.
#   (fx, fy, 시작줌, 끝줌)   fx/fy: 0=왼쪽·위, 0.5=가운데, 1=오른쪽·아래
KENBURNS_FRAMINGS = (
    (0.50, 0.45, 1.00, 1.18),   # @0.2  전체, 천천히 확대
    (0.50, 0.22, 1.32, 1.42),   # @1.0  위쪽 타이트 (얼굴)
    (0.30, 0.45, 1.12, 1.22),   # @1.8  왼쪽으로 치우침
    (0.70, 0.45, 1.12, 1.22),   # @2.6  오른쪽으로 치우침
    (0.50, 0.60, 1.38, 1.16),   # @3.4  아래쪽 타이트 -> 줌아웃
)
FRAMING_STEP = 0.8              # @초를 프레이밍 번호로 바꿀 때의 간격
