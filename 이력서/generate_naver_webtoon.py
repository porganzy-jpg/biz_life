...
IMG_DIR = r"C:/Users/itzia/biz_life/이력서/images"
...
add_styled_paragraph(doc, "NAVER WEBTOON — Global Webtoon Data Analyst 지원", size=Pt(14), bold=False,
...
add_styled_paragraph(doc, "Senior Data Analyst  |  디지털 콘텐츠 × 글로벌 서비스 × 실험 기반 성장", size=Pt(13),
...
summary_p1 = (
    "전 세계 수천만 사용자가 이용하는 디지털 콘텐츠 서비스에서, KPI를 정의하고 실험을 설계하며 "
    "데이터로 성장을 견인하는 것 — 이것이 지난 15년간 제가 해온 일이며, "
    "NAVER WEBTOON Global Webtoon Data Analyst 포지션에 지원하는 이유입니다."
)
summary_p2 = (
    "현재 Tving(OTT)에서 디지털 콘텐츠 서비스의 핵심 KPI를 정의하고, AI 기반 자동화 대시보드를 구축하며, "
    "콘텐츠 실적 예측 모델을 운영하고 있습니다. '구독자가 어떤 콘텐츠를 왜 소비하고, 무엇이 이탈을 막는가'를 "
    "데이터로 답하는 체계를 직접 설계했습니다. 이 경험은 웹툰이라는 디지털 콘텐츠 플랫폼에서 "
    "'어떤 작품이 유저를 유입시키고, 어떤 BM이 결제를 견인하는가'를 분석하는 데 직접 전이될 수 있습니다."
)
summary_p3 = (
    "그 이전 Coupang에서는 3P Marketplace BI Principal로서 AB Test 플랫폼(XPC)을 활용한 실험 설계·검증, "
    "AARRR 퍼널 기반 성장 분석, 17만+ 상품의 프로모션 전환 전략을 데이터로 수립했습니다. "
    "eBay Korea에서는 3개 사이트(Gmarket, Auction, G9)의 행동데이터 수집 시스템을 기획하고 "
    "데이터 거버넌스를 0에서 구축한 경험이 있어, 신규 서비스의 로그·데이터 테이블 설계에도 강점이 있습니다."
)
summary_p4 = (
    "기술(SQL, Python, Tableau, Airflow)을 도구로 쓰되, 항상 비즈니스 질문에서 출발하여 "
    "실행 가능한 전략으로 귀결시키는 것이 저의 분석 철학입니다. "
    "NAVER WEBTOON의 글로벌 사업 성장을 데이터로 가속화하는 데 기여하고 싶습니다."
)
...
add_styled_paragraph(doc, summary_p3, size=Pt(10.5), line_spacing=1.5,
                    space_before=Pt(0), space_after=Pt(10))
add_styled_paragraph(doc, summary_p4, size=Pt(10.5), line_spacing=1.5,
                    space_before=Pt(0), space_after=Pt(16))
...
add_styled_paragraph(doc, "NAVER WEBTOON이 원하는 역량 → 제가 증명한 경험",
                    size=Pt(10), color=MEDIUM_GRAY, space_after=Pt(10))

fit_data = [
    ["요구 역량", "보유 경험"],
    ["핵심 KPI 정의 및\n대시보드 구현·고도화",
     "Tving: 구독자·시청시간·재구독률 등 핵심 KPI 정의 + AI 자동화 대시보드 구축\n"
     "Coupang: 셀러 GMV, FEP 전환율 등 다차원 KPI 대시보드 (Tableau)\n"
     "eBay: BI 서비스 플랫폼, 검색 키워드·프로모션 실적 분석 플랫폼 구축"],
    ["실험 설계 및\n통계 기반 효과 검증",
     "Coupang: XPC 플랫폼 기반 AB Test 수십 건 설계·실행·검증\n"
     "FEP 가격 최적화, MyShop 묶음구매 UX, Auto Price 효과 검증 등\n"
     "eBay: Deep Link Gate 유입 케이스별 AB Test 설계·실행"],
    ["앱/웹 로그 데이터 활용\n퍼널 분석",
     "Coupang: 셀러 AARRR 퍼널 (SignUp→Live→Active→GMV) End-to-End 구축\n"
     "Tving: 구독 퍼널 + 시청 퍼널 분석, 이탈 최다 단계 데이터 식별\n"
     "eBay: Funnel 기반 도메인별 이탈/전환 분석"],
    ["신규 서비스\n데이터 구조 설계",
     "eBay: Montelena TF (Owner PM, 1.5년) — 3개 사이트 행동데이터 수집 시스템 + 지표 표준화\n"
     "Coupang: GA → 내부 DB 연결 ETL 파이프라인 (Airflow) 직접 설계·구축\n"
     "Tving: AI 쿼리 시스템을 위한 전사 데이터 카탈로그 구조화"],
    ["디지털 콘텐츠 산업 이해\n(우대)",
     "Tving (현재): OTT 디지털 콘텐츠 서비스 데이터 분석 팀장\n"
     "콘텐츠 실적 예측, 구독자 행동 분석, 시청시간 기반 KPI 설계 등 직접 수행"],
    ["사업 전략 / 그로스해킹\n/ CRM (우대)",
     "Coupang: 셀러 성장 전략, 프로모션 최적화, 그로스 분석\n"
     "E-Land: 고객 세그먼트 기반 CRM, 타겟 마케팅 전략 수립"],
]
...
add_styled_paragraph(doc, "총 경력 15년+ | 리테일 → e-Commerce → 디지털 콘텐츠(OTT) | KPI 설계 · 실험 · 퍼널 · 대시보드 · 데이터 인프라",
                    size=Pt(10), color=MEDIUM_GRAY, space_after=Pt(10))
...
comp_data = [
    ("Technical Skills", "SQL (Hive/Oracle/MySQL) — 분석 전 과정 주도 수행, Tableau, Python, "
     "Google Analytics, Apache Airflow (ETL), ElasticSearch, Kibana, R, D3.js"),
    ("Experimentation", "AB Testing (Coupang XPC 플랫폼), 통계 기반 효과 검증, 실험 설계 프레임워크, "
     "프로덕트·BM 개선 실험, 의사결정 지원 리포트"),
    ("Growth Analytics", "Funnel Analysis, AARRR Framework, Cohort Analysis, KPI 정의·설계, "
     "대시보드 구현·고도화, 앱/웹 로그 분석, CRM, 세그먼트 분석"),
    ("Data Infra & AI", "행동데이터 수집 시스템 설계, 데이터 거버넌스 표준화, 로그·데이터 테이블 설계, "
     "AI 자동화 대시보드, 예측 모델링, LLM 챗봇"),
]
...
add_project_image(doc, os.path.join(IMG_DIR, "tving_ai_dashboard.jpg"),
                  "[그림] AI 지능형 대시보드 — AI-Dave 자동 인사이트 생성 & 챗봇 통합 화면")
...
add_project_image(doc, os.path.join(IMG_DIR, "tving_kpi_breakdown.jpg"),
                  "[그림] KPI 포뮬러 분해 대시보드 — Watch Hour = User Traffic × 시청 전환율 × 시청시간/전환")
...
awards_table = doc.add_table(rows=4, cols=4)
...
awards_data = [
    ["2023", "마켓플레이스 부문 우수자 (상금 100만 원)", "23년 상반기 3P Marketplace BI 성과 우수", "Coupang"],
...
timeline_data = [
    ...
    ("2023", "마켓플레이스 부문 우수자 수상", "상금 100만 원 — BI 성과 우수"),
...
closing_text = (
    "디지털 콘텐츠 서비스에서 KPI를 정의하고 대시보드를 구축하며, 실험을 설계하고 퍼널을 분석하여 "
    "비즈니스 성장을 데이터로 견인해 온 15년의 경험을 NAVER WEBTOON에서 펼치고 싶습니다. "
    "전 세계 수천만 유저가 즐기는 웹툰 서비스의 데이터를 통해, "
    "사업 성장을 가속화하고 전략적 방향성을 제시하는 데 기여하겠습니다."
)
...
#  SAVE
# ════════════════════════════════════════════════════════════════
output_path = r"C:/Users/itzia/biz_life/이력서/NAVER_WEBTOON_연새한솔_경력기술서_v2.docx"
